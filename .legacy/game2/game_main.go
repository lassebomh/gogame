...
func Start() {
	...
	for pos, chunk := range g.Level.Chunks {
		...
		for x := range CHUNK_WIDTH {
			for z := range CHUNK_WIDTH {
				for y := range CHUNK_HEIGHT {
					if (chunk[x][z][y].Faces != [4]Face{}) || (chunk[x][z][y].Ground != Ground{}) {
						usedCells++
					}
				}
			}
		...
		if usedCells == 0 {
			delete(g.Level.Chunks, pos)
		}
	}

	save := g.ToSave()

	if err := save.WriteToFile(saveFileName); err != nil {
		log.Fatal(err)
	}
