...
void main() {
  float dither = fract(sin(dot(gl_FragCoord.xy/8, vec2(12.9898, 78.233))) * 43758.5453) * 2;
  dither -= 1;
  
  float trans = pow(iTransition, 4);
  
  vec2 uv = gl_FragCoord.xy / iResolution;
  
  vec2 uvoffset = vec2(0);
  if ((int(gl_FragCoord.x) & 1) == 1) {
    uvoffset.x = dither/5;
  }
  if ((int(gl_FragCoord.y) & 1) == 1) {
    uvoffset.y = dither/10;
  }
  
  vec4 col0 = texture(iChannel0, uv);
  
  
  vec4 col1 = texture(iChannel1, uv + uvoffset * (1-trans));
  col0.xyz = vec3((col0.x + col0.y + col0.z) / 3);
  
  finalColor = mix(col0, col1, trans);
